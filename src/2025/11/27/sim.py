...
class Simulation:
    ...
    def record_queue(self, which):
        if which == "dev":
            queue = self.dev_queue
        else:
            queue = self.test_queue
        for priority in range(len(self.params["task_priorities"])):
            num = sum(1 if x.priority == priority else 0 for x in queue.items)
            self.queue_log.append(
                {
                    "time": log_fmt(self.now),
                    "queue": which,
                    "priority": priority,
                    "count": num,
                }
...
class Task(Labeled):
    ...
    def __init__(self, sim):
        ...
        super().__init__()
        self["priority"] = sim.task_priority()
